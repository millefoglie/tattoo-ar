...
class AugmentedImageRecognizer(
    ...
    companion object {
        ...
        private const val TATTOO_IMAGE_FILE = "tattoo_2k.jpg"
        val TAG = AugmentedImageRecognizer::class.simpleName
    }

    private var scheduler: DelayedCallbackScheduler? = null
    private var camera: CameraDevice? = null
    private val trackers = ArrayList<ImageTracker>()
    private var bgRenderer: BackgroundRenderer? = null
    private var tattooRenderer: TattooRenderer? = null
    ...
    fun initGL() {
        ...
        val textureBitmap = BitmapFactory.decodeStream(context?.assets?.open(TATTOO_IMAGE_FILE))
        tattooRenderer = TattooRenderer(textureBitmap)
                ...
                if (result != null) {
                    for (targetInstance in result.targetInstances()) {
                        ...
                        if (status == TargetStatus.Tracking) {
                            ...
                            tattooRenderer?.render(projectionMatrix, targetInstance.pose(), scale)
